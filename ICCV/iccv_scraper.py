...
从 CVF Open Access 抓取指定年份的 ICCV 论文，输出 CSV 与 .bib（向官方 bib 注入摘要）。
示例：
    python iccv_scraper.py --year 2025 --outdir ./iccv2025
    python iccv_scraper.py --year 2024 --outdir ./iccv2024 --csv-full
...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; ICCV-Scraper/1.0; +https://example.org/academic-use)",
...
def find_listing_url(year: int) -> str:
    """
    入口优先尝试：
      1) https://openaccess.thecvf.com/ICCV{year}?day=all
      2) https://openaccess.thecvf.com/ICCV{year}
    """
    candidates = [
        f"{BASE}ICCV{year}?day=all",
        f"{BASE}ICCV{year}",
    ]
    for u in candidates:
        try:
...
def build_csv_row_full(
    ...
    return {
        ...
        "booktitle": str(fields.get("booktitle", "ICCV")),
...
def run(year: int, outdir: str, csv_name: Optional[str], bib_name: Optional[str], csv_full: bool) -> None:
    ...
    csv_path = os.path.join(outdir, csv_name or (f"iccv{year}_full.csv" if csv_full else f"iccv{year}.csv"))
    bib_path = os.path.join(outdir, bib_name or f"iccv{year}.bib")
        ...
        try:
            ...
            if official_bib:
                ...
                if fetched_bib:
                    ...
                    bib_key = guess_bib_key_from_bib(fetched_bib) or f"ICCV{year}_" + hashlib.md5((paper_url).encode()).hexdigest()[:8]
                    ...
                    bib_key = f"ICCV{year}:" + hashlib.md5((title + paper_url).encode("utf-8")).hexdigest()[:8]
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Scrape ICCV proceedings (CVF Open Access) for a given year.")
    parser.add_argument("--year", type=int, default=2025, help="ICCV year, e.g., 2025")
