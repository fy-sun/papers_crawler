...
功能：
  ...
  - 下载其 pdf_url 指向的 PDF；
  - 规范化重命名文件，并加上形如：([C]-2025-ICCV) 的前缀。
...
参数说明：
  ...
  --year        前缀中的年份，默认 2025
  --conf        前缀中的会议简称，默认 ICCV（可改为 ICCV / ECCV / 其它）
...
MULTI_or_SEMSEG_REGEX_4 = re.compile(MULTI_or_SEMSEG_PATTERN, FLAGS)
...
def main():
    ...
    ap.add_argument("--csv", default="./files/iccv2025_full.csv", help="Input CSV path (must contain columns: title, abstract, pdf_url).")
    ...
    ap.add_argument("--conf", type=str, default="ICCV", help="Conference token used in prefix (default: ICCV).")
    ap.add_argument(
        "--prefix",
        type=str,
        default="([C]-{year}-{conf})",
